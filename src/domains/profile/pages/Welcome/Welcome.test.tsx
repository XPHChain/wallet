...
describe("Welcome", () => {
	it("should render with profiles", () => {
		...
		expect(history.location.pathname).toStrictEqual(profileDashboardUrl);
	...
	it("should navigate to profile dashboard", () => {
		...
		expect(history.location.pathname).toStrictEqual(`/profiles/${profile.id()}/dashboard`);
		expect(asFragment()).toMatchSnapshot();
	});
	...
	it("should navigate to profile settings with correct password", async () => {
		...
		await waitFor(() => {
			expect(history.location.pathname).toStrictEqual(`/profiles/${profile.id()}/settings`);
		});
