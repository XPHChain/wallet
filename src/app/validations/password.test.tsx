...
describe("Password Validation", () => {
	...
	it("should require at least 1 lowercase character", async () => {
		...
		await expect(passwordValidation.password().validate("NO_LOWER")).resolves.toStrictEqual(
	...
	it("should require at least 1 uppercase character", async () => {
		...
		await expect(passwordValidation.password().validate("no_upper")).resolves.toStrictEqual(
			t("COMMON.VALIDATION.PASSWORD_WEAK"),
		);
		expect(pwnd).not.toHaveBeenCalled();
	});
	...
	it("should require at least 1 numeric character", async () => {
		...
		await expect(passwordValidation.password().validate("NoNumeric")).resolves.toStrictEqual(
			t("COMMON.VALIDATION.PASSWORD_WEAK"),
		);
		expect(pwnd).not.toHaveBeenCalled();
	});
	...
	it("should require at least 1 special character", async () => {
		...
		await expect(passwordValidation.password().validate("N0SpecialChar5")).resolves.toStrictEqual(
			t("COMMON.VALIDATION.PASSWORD_WEAK"),
		);
		expect(pwnd).not.toHaveBeenCalled();
	});
	...
	it("should be at least 8 characters long", async () => {
		...
		await expect(passwordValidation.password().validate("shortpw")).resolves.toStrictEqual(
			t("COMMON.VALIDATION.PASSWORD_WEAK"),
		);
		expect(pwnd).not.toHaveBeenCalled();
	});
	...
	it("should require the password not to have been leaked", async () => {
		...
		await expect(passwordValidation.password().validate("S3cUr3!Pas#w0rd")).resolves.toStrictEqual(
	...
	it("should require different password than the old password", async () => {
		...
		await expect(passwordRule.validate("S3cUr3!Pas#w0rd")).resolves.toStrictEqual(
	...
	it("should fail on password and confirm password mismatch", () => {
		...
		expect(confirmPassword.validate("password2")).toStrictEqual(t("COMMON.VALIDATION.PASSWORD_MISMATCH"));
	});
	...
	it("should require password to be entered before confirm password", () => {
		...
		expect(confirmPassword.validate("password2")).toStrictEqual(
	...
	it("should confirm optional password", () => {
		...
		expect(confirmPassword.validate()).toBe(true);
		expect(confirmPassword.validate("password")).toStrictEqual(
			t("COMMON.VALIDATION.FIELD_REQUIRED", {
				field: t("SETTINGS.GENERAL.PERSONAL.PASSWORD"),
			}).toString(),
		);
	...
	it("should fail validation if optional password is set", () => {
		...
		expect(confirmPassword.validate()).toStrictEqual(t("COMMON.VALIDATION.PASSWORD_MISMATCH"));
		expect(confirmPassword.validate("test2")).toStrictEqual(t("COMMON.VALIDATION.PASSWORD_MISMATCH"));
	});
});
